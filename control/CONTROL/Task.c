...
// 初始化：记录初始航向角，设置初始速度
    ...
    case B_POINT_DELAY:
        // B点LED提示延时状态（非阻塞）
        delay_counter++;
        if (delay_counter >= (TASK3_B_PONIT_DELAY_TIME / 5))
        { // 5ms中断，100次约等于500ms
            led2_off();
    ...
    case A_POINT_DELAY:
        // A点任务完成LED提示延时状态（非阻塞）
        delay_counter++;
        if (delay_counter >= (TASK3_INTERVAL / 5))
        { // 5ms中断，TASK3_INTERVAL次约等于TASK3_INTERVALms
            currentState = COMPLETED;
        }
        return 0;
...
u8 Task4_Handler(void)
{
    ...
    case B_POINT_DELAY:
        // B点LED提示延时状态（非阻塞）
        delay_counter++;
        if (delay_counter >= (Task4_B_Point_Delay_Time / 5))
        { // 使用全局变量
            led2_off();
            printf("在B点旋转至目标角度: %.2f\r\n", targetAngle);
            currentState = TURN_AT_B;
    ...
    case A_POINT_DELAY:
        // A点任务完成LED提示延时状态（非阻塞）
        delay_counter++;
        if (delay_counter >= (Task4_Interval / 5))
        { // 使用全局变量
